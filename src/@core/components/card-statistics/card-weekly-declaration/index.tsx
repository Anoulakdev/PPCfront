...
import { CardWeeklyDeclarProps } from 'src/@core/components/card-statistics/types'

import Icon from 'src/@core/components/icon'
import CustomAvatar from 'src/@core/components/mui/avatar'
import { Divider, LinearProgress } from '@mui/material'
import { Declaration } from '../../../../lib/__generated__/graphql'
...
const CardWeeklyDeclar = (props: CardWeeklyDeclarProps) => {
  const {
    sx,
    title2,
      ...
      <CardContent sx={{ p: theme => `${theme.spacing(5)} !important` }}>
        ...
        <Box sx={{ mb: 3.5, gap: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Box sx={{ py: 2.25, display: 'flex', alignItems: 'flex-end', flexDirection: 'column' }}>
            <Box sx={{ mb: 2.5, display: 'flex', alignItems: 'center' }}>
              <Typography sx={{ mr: 1.5, color: 'text.secondary' }}>Lastweek:{lastWeek}</Typography>
            </Box>
...
export default CardWeeklyDeclar
