...
import { DAILYDISPATCHS, WEEK_ALL_DOCUMENT, WEEK_DIS, WEEK_POWER_PURCHASE } from '@/lib/query'
...
const defaultColumns: GridColDef[] = [
  {
    ...
    renderCell: ({ row }: CellType) => (
      ...
      >{`${row.powerNo}-${row.weekId}`}</Typography>
    )
  },
  {
    flex: 0.1,
...
const DailyReport = () => {
  ...
  // Query
  const { data, loading: loadingData } = useQuery(WEEK_ALL_DOCUMENT, {
    variables: {
      queryInput: {
  ...
  const allDocument = data?.allWeeklyDocument ? data?.allWeeklyDocument : []
