...
import { MONTH_ACKNOWLEDGED_DIS } from '@/lib/mutation'
...
const Monthdadform = () => {
  ...
  // ** Mutation
  const [acknowledgedMonthDispatch, { loading: createLoading }] = useMutation(MONTH_ACKNOWLEDGED_DIS, {
    onCompleted: data => {
      handleClose()
  ...
  const handleSubmit = async () => {
    ...
    // console.log(total())
    try {
      await acknowledgedMonthDispatch({
        variables: {
          acknowledgedMonthDispatch: newData
        }
      })
    } catch (error) {
      console.log(error)
      ...
      <Grid item xs={12}>
        <Card>
          ...
          >
            ...
            <Typography sx={{ textAlign: 'center', fontWeight: 'bold' }}>Monthly Declaration</Typography>
          ...
          <Box className='demo-space-x' sx={{ mb: 5, padding: '10px' }}>
            ...
            >
              {loading ? 'Loading...' : 'Acknowledged Dispatch'}
            </Button>
          </Box>
        </Card>
      </Grid>
