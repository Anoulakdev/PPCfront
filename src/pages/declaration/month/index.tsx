...
import { MONTH_DEC, MONTH_SUMMARY_ENERGY } from '@/lib/query'
...
const MonthlyDispatch = () => {
  ...
  // Query
  const { data, loading: loadingData } = useQuery(MONTH_DEC, {
    variables: {
      queryInput: {
  ...
  const monthDis = data?.monthDeclarations ? data?.monthDeclarations : []
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Tooltip title='View'>
            <IconButton
              ...
              href={`/monthly-declaration/${row._id}`}
            >
      ...
      <Grid container spacing={6}>
        ...
        <Grid item xs={12}>
          <Card>
            ...
            >
              <Grid item>
                <Link href='/declaration/month/create' passHref>
